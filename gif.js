// gif.js - Discord GIF Bot (Render-ready)
const { 
  Client, 
  GatewayIntentBits, 
  Events, 
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  if (interaction.isChatInputCommand() && interaction.commandName === 'picker') {
    const gifs = getGifs();
    if (!gifs.length) {
      return interaction.reply({ content: 'No GIFs available.', ephemeral: true });
    }
    ...
    const attachments = gifs.map(
      gif => new AttachmentBuilder(path.join(gifsFolder, gif), { name: gif })
    );

    gifs.forEach(gif => {
      embed.addFields({ 
        name: gif.replace('.gif',''), 
        value: `[Preview](attachment://${gif})`, 
        inline: true 
      });
    });
  ...
  if (interaction.isButton() && interaction.customId.startsWith('gif_')) {
    ...
    if (!fs.existsSync(gifPath)) {
      return interaction.reply({ content: 'GIF not found.', ephemeral: true });
    }

    await interaction.channel.send({ 
      content: `${interaction.user} sent this:`, 
      files: [gifPath] 
    });
...
// Login bot with Render environment variable
